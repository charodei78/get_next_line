...
/*   Updated: 2020/05/14 15:46:46 by hheimerd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					ft_find_elem(int fd, t_list **start,
	t_list **res, char **line)
{
	t_list			*tmp_list;

	if (!(tmp_list = *start))
	{
		*start = ft_create_elem(fd, NULL, NULL);
		*res = *start;
		return (*res ? 0 : -1);
	}
	while (tmp_list)
	{
		...
		{
			tmp_list->next = ft_create_elem(fd, NULL, tmp_list);
			*res = tmp_list->next;
			return (*res ? 0 : -1);
		}
	}
